...
AmrPipeline::AmrPipeline(VkDevice device, VkExtent2D swapChainExtent, VkPipelineLayout pipelineLayout, VkRenderPass renderPass)
	:m_device(device)
{
	AmrShader vertShader(device, "shaders/vert.spv");
	AmrShader fragShader(device, "shaders/frag.spv");
	...
	auto bindingDescription = AmrVertex::getBindingDescription();
	auto attributeDescriptions = AmrVertex::getAttributeDescriptions();
